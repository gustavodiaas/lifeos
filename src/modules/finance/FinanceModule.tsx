...
export function FinanceModule() {
      ...
      if (editingLancamento) {
        if (mode === 'all' && editingLancamento.grupo_recorrencia_id) {
          const { error: err } = await supabase
            .from('lancamentos')
            .update({ tipo: data.tipo, valor: data.valor, descricao: data.descricao, categoria: data.categoria })
            .eq('grupo_recorrencia_id', editingLancamento.grupo_recorrencia_id);
          if (err) throw err;
          setLancamentos((prev) =>
            prev.map((l) =>
              l.grupo_recorrencia_id === editingLancamento.grupo_recorrencia_id ? { ...l, ...data } : l
            )
          );
        } else {
  ...
  const handleRemoveClick = (id: string) => setDeleteConfirmId(id);
  ...
  const TABS: { id: Tab; label: string; Icon: React.FC<any> }[] = [
    ...
    { id: 'totais',   label: 'Totais',   Icon: PieChart },
    { id: 'tags',     label: 'Tags',     Icon: Layers },
    { id: 'horizon',  label: 'Previsão', Icon: LayoutGrid },
    { id: 'planilha', label: 'Planilha', Icon: Table },
  ];

  return (
    /*
     * Layout de finanças: ocupa 100% da altura disponível (h-full),
     * divide em header fixo + conteúdo com scroll próprio.
     * Isso elimina o scroll duplo da página no mobile.
     */
    <div className="h-full flex flex-col w-full max-w-6xl mx-auto relative">
      {toastMessage && (
        <ErrorToast message={toastMessage} onClose={() => setToastMessage(null)} />
      )}

      {/* ── Header Fixo: Mês + Tabs ───────────────────────────────── */}
      <div className="shrink-0 px-4 pt-2 pb-0 md:px-6 md:pt-4">
        <div className="glass-card p-2.5 flex flex-col sm:flex-row items-center justify-between gap-2 border border-white/10 shadow-sm rounded-2xl backdrop-blur-xl">
          {/* Navegação de Mês */}
          <div className="flex items-center justify-between w-full sm:w-auto gap-2">
            ...
            <button
              onClick={() => { setEditingLancamento(null); setModalError(null); setModalOpen(true); }}
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-4 pt-3 pb-4 md:px-6 md:pb-6">
        {loading ? (
          ...
          <>
            ...
            {activeTab === 'extrato' && (
              <AppleFinanceView
                ...
                balance={netBalance}
                onNewTransaction={() => { setEditingLancamento(null); setModalError(null); setModalOpen(true); }}
      ...
      <TransactionModal
        open={modalOpen}
        onClose={() => { setModalOpen(false); setEditingLancamento(null); }}
        onSave={handleSave}
